...
<div class="app-sidebar-logo px-6" id="kt_app_sidebar_logo">
    ...
    @if (config('app.logo') != null)
        <a href="{{ route('dashboard') }}">
            <img alt="Logo" src="{{ asset(config('app.logo')) }}" class="h-25px app-sidebar-logo-default" />
            <img alt="Logo" src="{{ asset(config('app.logo')) }}" class="h-20px app-sidebar-logo-minimize" />
        </a>
    @else
       <a href="{{ route('dashboard') }}">
            <h2 class="logo-title mb-0">{{config('app.name')}}</h2>
       </a>
    @endif
    <!--end::Logo image-->
